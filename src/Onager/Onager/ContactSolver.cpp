	...
	void solveContacts(WorldContext* w, Contact** contacts, int numContacts, ContactConstraint* constraints)
	{
		for (int j = 0; j < numContacts; ++j)
		{
			...
			for (int i = 0; i < man->numPoints; ++i)
			{
				...
				// impulse to resolve penetration
				//float bias = -0.2f * invDt * MIN(0.0f, (man->points[i].penetration + 0.15f));
				...
				w->p[idxA].a -= cross(c->rA[i], impulse);
				w->p[idxB].a += cross(c->rB[i], impulse);
